...
def get_coords(p):
    ...
    for i in range(1, p.total_residue() + 1):
        ...
        if residue.is_protein():
            protein_residues.append((i, residue))

    if not protein_residues:
        raise ValueError(
            "pose contains no protein residues after PDB import; "
            "the input is likely empty to PyRosetta or all occupancies are zero"
        )

    def collect_anchor(atom_name):
        coords = []
        missing = []
        for pose_index, residue in protein_residues:
            if not residue.has(atom_name):
                missing.append(f"{pose_index}:{residue.name()}")
                continue
            coords.append(np.array(residue.atom(atom_name).xyz(), dtype=np.float32))
        if missing:
            sample = ", ".join(missing[:8])
            raise ValueError(f"missing backbone atom {atom_name} for protein residues: {sample}")
        return np.stack(coords)

    # three anchor atoms to build local reference frame
    N = collect_anchor('N')
    Ca = collect_anchor('CA')
    C = collect_anchor('C')
...
def set_features1D(pdict):
    ...
    mask1d = np.ones(nres).astype(bool)
    mask1d[0] = mask1d[-1] = 0
...
def get_interface_pair(structure, cutoff=15):
    atoms = Bio.PDB.Selection.unfold_entities(structure, "A") # 
    atoms = [a for a in atoms if a.id == 'CA']
    ns = Bio.PDB.NeighborSearch(atoms)
    close_residues = ns.search_all(cutoff, "R") 
    ...
    for res in close_residues:
        ...
        if res1.get_parent().id == res2.get_parent().id:
            continue
        if res1 not in res_set:
            res_dic[res1.get_parent().id].append(res1.get_id()[1])
            res_set.add(res1)
        if res2 not in res_set:
            res_dic[res2.get_parent().id].append(res2.get_id()[1])   
            res_set.add(res2)
...
def homo_index_interface(pdb_file_path, is_homo=False, pdb_id='', output_file_path=None, process_stage='train'):
    assert os.path.exists(pdb_file_path), f"{pdb_file_path} is not exists"

    # no output_file_path, use the deafult
    if not output_file_path:
        print('the output_file_path is None, use the default path is:{}'.format(pdb_id+'.pdb'))
        output_file_path = pdb_id+'.pdb'
        
    # get the structure
    parser = PDBParser()
    pdb_obj = parser.get_structure(file=pdb_file_path, id=pdb_id)
    all_features_dict = defaultdict(dict)
    seq_list = []
    chain_list=[]
    # get the full chain and the total len  
    for chain in pdb_obj[0].get_chains():
        chain_seq = ''.join([AA3_to_AA1.get(i.get_resname(), '') for i in chain])
        seq_list.append(chain_seq)
        chain_list.append(chain.id)
        all_features_dict[chain.id]['sequence'] = chain_seq
        all_features_dict[chain.id]['seq_length'] = len(chain_seq)    
    complex_length = sum([len(x) for x in seq_list])

    # whether is monomer , whether need to CUT_RESIDUE
    if len(seq_list) <= 1 :
        print('The input pdb is a monomer, will not cut residues')
        output_file_path = pdb_file_path
        CUT_RESIDUE = False
    else:
        CUT_RESIDUE = True

    # cut residue and get the interface residue
    if CUT_RESIDUE:
        interface_dic = get_interface_pair(pdb_obj)

    # whether need to asure the homo
    HOMO_FLAG = True
    if process_stage != 'train' or len(seq_list) > 2:
        asym_id_dict={}
        HOMO_FLAG = False
        for th, seq in enumerate(list(set(seq_list))) :
            asym_id_dict[seq]=th+1   

    # compute the all features
    entity_id = 1
    sym_id_dict={}
    cut_residue_complex = 0
    inter_fea_dict={
        'entity_id_inter':[],
        'res_index_inter':[],
        'seq_inter':[],
        'res_resort_index_inter':[],
        'sym_id_inter':[],
        'asym_id_inter':[]
    }

    for chain_id in chain_list:
        feature_dict=all_features_dict[chain_id]
        seq=feature_dict['sequence']
        # interface feature
        interface_residue_index = interface_dic[chain_id]
        interface_residue_index.sort()
        cut_residue_complex += len(interface_residue_index)
        assert len(interface_residue_index) > 3, f"%s no need to process, the interface is too small"%(pdb_file_path)

        inter_fea_dict['entity_id_inter']+=(list(entity_id * np.ones(len(interface_residue_index))))
        inter_fea_dict['res_index_inter']+=(interface_residue_index)
        inter_fea_dict['seq_inter']+=[seq[i] for i in range(len(seq)) if i in interface_residue_index]
        inter_fea_dict['res_resort_index_inter']+=list(np.arange(1,len(interface_residue_index)+1,1))
        
        if HOMO_FLAG:
            if is_homo:
                # interface feature
                feature_dict['sym_id_interface'] = entity_id * np.ones(len(interface_residue_index))
                feature_dict['asym_id_interface'] = 1 * np.ones(len(interface_residue_index)) 
            else: 
                # interface feature
                feature_dict['sym_id_interface'] = entity_id * np.ones(len(interface_residue_index))
                feature_dict['asym_id_interface'] = 1 * np.ones(len(interface_residue_index))                  
        else:
            if seq not in sym_id_dict.keys():
                sym_id_dict[seq] = 1
            else:     
                sym_id_dict[seq] += 1
            # interface feature
            ...
            feature_dict['asym_id_interface'] = 1 * np.ones(len(interface_residue_index)) 
        entity_id += 1
        inter_fea_dict['sym_id_inter']+=(list(feature_dict['sym_id_interface']))
        inter_fea_dict['asym_id_inter']+=(list(feature_dict['asym_id_interface']))

    inter_fea_dict['residue_index_multimer'] = np.arange(1,cut_residue_complex+1,1)   
    
    # save the cut interface pdb
    # contact_structure = PDB.Structure.Structure("interface_protein")
    # new_model = PDB.Model.Model(0)

    # for th, c in enumerate(chain_list):
    #     k=c
    #     v=interface_dic[c]
    #     chain = pdb_obj[0][k]
    #     new_chain = PDB.Chain.Chain(k)

    #     for residue in chain:
    #         if residue.get_id()[1] in v:
    #             new_chain.add(residue.copy())
    #     new_model.add(new_chain)

    # contact_structure.add(new_model)
    # io = PDBIO()
    # io.set_structure(contact_structure)
    # io.save(output_file_path)
    return output_file_path, inter_fea_dict

def index_fea(inter_fea_dict):
    
    index_list=[]
    for k in K_Index:
        index_list.append(inter_fea_dict[k])   
...
def get_feat(pose):
    ...
    length =  int(pose.total_residue())
    x_p = np.zeros((length, 7))
    for i in range(length):
        aas = pose.residue(i+1).name()[:3]
        if  aas == 'ALA':
            x_p[i] = all_for_assign[0,:]
        elif aas == 'CYS':
            x_p[i] = all_for_assign[1,:]
        elif aas == 'ASP':
            x_p[i] = all_for_assign[2,:]
        elif aas == 'GLU':
            x_p[i] = all_for_assign[3,:]
        elif aas == 'PHE':
            x_p[i] = all_for_assign[4,:]
        elif aas == 'GLY':
            x_p[i] = all_for_assign[5,:]
        elif aas == 'HIS':
            x_p[i] = all_for_assign[6,:]
        elif aas == 'ILE':
            x_p[i] = all_for_assign[7,:]
        elif aas == 'LYS':
            x_p[i] = all_for_assign[8,:]
        elif aas == 'LEU':
            x_p[i] = all_for_assign[9,:]
        elif aas == 'MET':
            x_p[i] = all_for_assign[10,:]
        elif aas == 'ASN':
            x_p[i] = all_for_assign[11,:]
        elif aas == 'PRO':
            x_p[i] = all_for_assign[12,:]
        elif aas == 'GLN':
            x_p[i] = all_for_assign[13,:]
        elif aas == 'ARG':
            x_p[i] = all_for_assign[14,:]
        elif aas == 'SER':
            x_p[i] = all_for_assign[15,:]
        elif aas == 'THR':
            x_p[i] = all_for_assign[16,:]
        elif aas == 'VAL':
            x_p[i] = all_for_assign[17,:]
        elif aas == 'TRP':
            x_p[i] = all_for_assign[18,:]
        elif aas == 'TYR':
            x_p[i] = all_for_assign[19,:]
    feat.append(x_p)
    feat = np.array(feat)
    return feat
...
def get_adj(pose):
    ...
    for i in range(length - 2):
        for j in range(i + 2, length):
            if dist_adj(pose.residue(i+1).xyz("CA"), pose.residue(j+1).xyz("CA")) < threshold:
                contacts.append((i, j))
    adj = np.array(contacts)
    return adj
...
def process(args):
    ...
    # inter_pdb, homo_index_dict=homo_index_interface(
    #     model, 
    #     output_file_path=inter_pdb, 
    #     # is_homo=is_homo
    # )

    # homo_intex=index_fea(homo_index_dict)
    
    # try:
    start_time = time.time()
    pose = Pose()

    pose_from_file(pose, model)
    fa_scorefxn = get_fa_scorefxn()
    ...
    num_nodes_from_feat = feat.shape[1] # feat 形状是 (1, N, F)
    max_index_in_adj = -1
    if adj.size > 0:
        max_index_in_adj = np.max(adj)
    
    # print("\n\n################# DEBUG INFO FROM FEATURIZER #################")
    # print(f"PDB Model: {model}")
    # print(f"pose.total_residue(): {pose.total_residue()}")
    # print(f"pose.size(): {pose.size()}")
    # print(f"Shape of 'feat' array: {feat.shape}")
    # print(f"Number of nodes derived from 'feat': {num_nodes_from_feat}")
    # print(f"Shape of 'adj' array: {adj.shape}")
    # print(f"Max index found in 'adj' array: {max_index_in_adj}")
    
    # if max_index_in_adj >= num_nodes_from_feat:
    #     print("!!!!!! CRITICAL WARNING: Index out of bounds DETECTED before saving! !!!!!!")
    # else:
    #     print(">>>>>> Data seems consistent before saving. (max_index < num_nodes)")
    # print("##############################################################\n\n")
    # ############# DEBUGGING BLOCK END #############
    
    print("即将保存:", inter_fea)
    print("保存内容 shapes:", 
        pdict['phi'].shape, 
        pdict['psi'].shape, 
        feat.shape, 
    ...
    np.savez_compressed(inter_fea,
        ...
        # homo_intex=homo_intex.astype(np.float16)
        )
    return True
    ...
    #     return False
    # except Exception as inst:
    #     print(f"❌ Error while processing {args}: {type(inst).__name__}: {inst}")
    #     import traceback
    #     traceback.print_exc()
    #     return False
...
def process_from_pose(pose):
    ...
    try:
        ...
        print("While processing", inter_fea+":", inst)
