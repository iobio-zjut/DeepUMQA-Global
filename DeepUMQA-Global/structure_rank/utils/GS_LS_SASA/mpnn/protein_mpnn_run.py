...
def main(args):
    ...
    #print("1111111111111111111111111111111111111111111111111111111111model_folder_path",model_folder_path)
    ...
    model.to(device)
    model.load_state_dict(checkpoint['model_state_dict'])#把 checkpoint 字典中保存的模型参数状态加载到 model 这个模型实例中，这样 model 就具有了这些预定义的参数，可用于后续的推理、继续训练等操作 。
    model.eval()
    ...
    with torch.no_grad():
        ...
        for ix, protein in enumerate(dataset_valid):
            ...
            pssm_log_odds_mask = (pssm_log_odds_all > args.pssm_threshold).float() #1.0 for true111, 0.0 for false
            ...
            if args.score_only:
                ...
                randn_1 = torch.randn(chain_M.shape, device=X.device)#生成与张量X维度相同的随机数

                log_probs = model(X, S, mask, chain_M*chain_M_pos, residue_idx, chain_encoding_all, randn_1)#torch.Size([1, 429, 21])怀疑输出的是每个位置的氨基酸概率
                
                # print("log_probs",log_probs)
                # print("log_probs",log_probs.shape)
                # return
                mask_for_loss = mask*chain_M*chain_M_pos#那条链上的序列需要预测
                # print("mask_for_loss",mask_for_loss)
                # print("mask_for_loss",mask_for_loss.shape)
                return
                ...
                with open(ali_file, 'w') as f:
                    for temp in temperatures:
                        for j in range(NUM_BATCHES):
                            ...
                            #print("11111111111111111111111111111111111111111111111111111111111111")
                            #log_probs ([1, 429, 21]) 
                            #print("S_sample",S_sample.shape,S_sample)
                            mask_for_loss = mask*chain_M*chain_M_pos
                            ...
                            #scores 是局部打分，只考虑需要设计的链上的氨基酸，用于评估生成的序列在设计区域的质量。
                            #global_scores 是全局打分，考虑整个蛋白质结构中的所有氨基酸，用于评估生成的序列在整个结构上的质量。
                            all_probs_list.append(sample_dict["probs"].cpu().data.numpy())
                            all_log_probs_list.append(log_probs.cpu().data.numpy())
                            #print("all_log_probs_list",all_log_probs_list)
                            S_sample_list.append(S_sample.cpu().data.numpy())
                            #包含了生成的氨基酸序列以及氨基酸的概率分布等信息
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--output_path", type=str, help="Path to a folder to output sequences, e.g. /home/out/")
    ...
    argparser.add_argument("--device", type=str, default='gpu', help="choose gpu cpu")
