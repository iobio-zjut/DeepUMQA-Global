...
def process_pdb_file(pdb_file):
    ...
    # 第一个命令
    os.system(f"./voronota-normal get-balls-from-atoms-file --annotated < {pdb_file_path} > {pdb_file_path}.txt")
    # 如果文件已经存在，则跳过执行下一个文件
    if os.path.exists(target_file_path):
        print(f"文件 {target_file_path} 已存在，跳过。")
    ...
    # 第二个命令
    os.system(f"./voronota-normal calculate-contacts --annotated < {pdb_file_path}.txt > {target_file_path}")
